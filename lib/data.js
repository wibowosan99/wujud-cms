...
export function waLink(settings, message) {
  return waLinkTo(settings.whatsapp_number, message || settings.whatsapp_message || 'Halo, saya ingin bertanya tentang paket di ' + (settings.site_name || ''));
}
...
export function getAffiliateBySlug(slug) {
  ...
  if (affiliate.valid_until) {
    const today = new Date().toISOString().slice(0, 10);
    if (affiliate.valid_until < today) return null;
  }
  return affiliate;
}
