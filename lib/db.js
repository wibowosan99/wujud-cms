...
function seedIfEmpty(db) {
  ...
  if (statsCount === 0) {
    const stmt = db.prepare('INSERT INTO stats (label, value, sort_order) VALUES (?,?,?)');
  ...
  if (packagesCount === 0) {
    const stmt = db.prepare(
      `INSERT INTO packages (title, slug, category, excerpt, content, image, price, duration, departure_date, featured, published, sort_order)
       VALUES (?,?,?,?,?,?,?,?,?,?,1,?)`
    );
    stmt.run(
      'Umroh Maulid Nabi',
  ...
  if (articlesCount === 0) {
    const stmt = db.prepare(
      `INSERT INTO articles (title, slug, category, excerpt, content, image, published)
       VALUES (?,?,?,?,?,?,1)`
    );
    stmt.run(
  ...
  if (teamCount === 0) {
    const stmt = db.prepare(
      `INSERT INTO team_members (name, position, photo, bio, sort_order, published) VALUES (?,?,?,?,?,1)`
    );
  ...
  if (testiCount === 0) {
    const stmt = db.prepare(
      `INSERT INTO testimonials (name, source, quote, photo, sort_order, published) VALUES (?,?,?,?,?,1)`
    );
  ...
  if (heroCount === 0) {
    const stmt = db.prepare(
      `INSERT INTO hero_slides (title, subtitle, image, cta_label, cta_link, sort_order, published) VALUES (?,?,?,?,?,?,1)`
    );
    stmt.run(
