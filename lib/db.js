...
function migrate(db) {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS affiliate_commissions (
      ...
      closed_at TEXT,
      payout_status TEXT NOT NULL DEFAULT 'belum_dicairkan',
      payout_date TEXT,
      notes TEXT,
      created_at TEXT DEFAULT (datetime('now'))
    );
  ...
  // Safe column migration: add columns to tables that existed before this feature,
  // without touching existing data.
  const affiliateCols = db.prepare("PRAGMA table_info(affiliates)").all().map((c) => c.name);  if (!affiliateCols.includes('email')) {
    db.exec("ALTER TABLE affiliates ADD COLUMN email TEXT");
  }
  if (!affiliateCols.includes('start_date')) {
    db.exec("ALTER TABLE affiliates ADD COLUMN start_date TEXT");
  }
  if (!affiliateCols.includes('end_date')) {
    db.exec("ALTER TABLE affiliates ADD COLUMN end_date TEXT");
    // migrate legacy valid_until column into end_date, if it exists from an earlier version
    if (affiliateCols.includes('valid_until')) {
      db.exec("UPDATE affiliates SET end_date = valid_until WHERE valid_until IS NOT NULL");
    }
  }
  // migrate legacy status values (pending/approved/rejected) + active flag into the
  // simplified aktif/nonaktif status, if this database predates the simplification
  if (affiliateCols.includes('active')) {
    db.exec(`
      UPDATE affiliates
      SET status = CASE WHEN status = 'approved' AND active = 1 THEN 'aktif' ELSE 'nonaktif' END
      WHERE status IN ('pending', 'approved', 'rejected')
    `);
  }

  if (!affiliateCols.includes('visit_count')) {
    db.exec("ALTER TABLE affiliates ADD COLUMN visit_count INTEGER NOT NULL DEFAULT 0");
  }

  const partnerLeadCols = db.prepare("PRAGMA table_info(partner_leads)").all().map((c) => c.name);
  if (!partnerLeadCols.includes('jamaah_estimate')) {
    db.exec("ALTER TABLE partner_leads ADD COLUMN jamaah_estimate TEXT");
  }
  if (!partnerLeadCols.includes('partner_type')) {
    db.exec("ALTER TABLE partner_leads ADD COLUMN partner_type TEXT");
  }
